    ...
    <div class="table-responsive mt-3">
        ...
        <table id="tabla_proyectos" class="table tableComon">
            ...
            $c3 = $mysqli->query("
                    ...
                    ots.id_obra as id_obra,
                    ps.concepto as tarea2,
                    ps.id_presupuesto_partidas as id_presupuesto_partidas, 
                    ps.concepto AS tarea, 
                    ots.fecha_vencimiento AS fecha
                    FROM empleados e
                    LEFT JOIN obras_trabajadores_subpartidas ots ON e.id = ots.id_empleado
                    LEFT JOIN presupuestos_subpartidas ps ON ots.id_presupuestos_subpartidas = ps.id
                    ORDER BY e.nombre, ots.fecha_vencimiento");
            while ($row = $c3->fetch_assoc()) {
                $concepto = $row['tarea'];
                $subpartida = $row['tarea2'];
                ...
                echo "<tr>
                        ...
                        <td>$subpartida</td>
                        <td>$trabajador</td>
                        <td><a target='_blank' href='obraPlanningDetailPartes2.php?id_presupuestos_subpartidas=$id_obra&id_obra=$id_obra&id_presupuestos_partidas=$id_presupuesto_partidas'><bi class='bi bi-arrow-right fs-6'></bi></a></td>
                      </tr>";
            }
