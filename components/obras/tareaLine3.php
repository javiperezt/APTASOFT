<tr class="<?=$fecha_vencimiento_subpartida < $today && !$is_checked ? "table-danger" : "";?>">
    <td>
    ...
    <td>
        <input onchange="updateGenerico('presupuestos_subpartidas','fecha_prox_intervencion',<?= $id_presupuestos_subpartidas; ?>, this.value)"
               type="date" class="form-control form-control-sm <?= $fecha_prox_intervencion && !$is_checked ? "bg-warning" : ""; ?>" value="<?= $fecha_prox_intervencion; ?>"></td>
    <?php
    if ($id_unidad_subpartida == 6) {
        ...
        ?>
        <td><?= $cantidad_presupuesto; ?><?= $simbolo_ud_material; ?></td>
        <?php
    }
    ?>
    <td><?= $total_horasRegistradas ? "$total_horasRegistradas" : "00:00"; ?>h</td>
    <td>
        <a href="obraPlanningDetailPartes2.php?id_presupuestos_subpartidas=<?= $id_presupuestos_subpartidas; ?>&id_obra=<?= $id_obra; ?>&id_presupuesto=<?= $id_presupuesto; ?>&id_presupuestos_partidas=<?= $id_presupuestos_partidas; ?>"><i
